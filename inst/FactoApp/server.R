shinyServer(function(input, output) {
  ...
  valeuretour <- function(){
    if(input$condesmethod!=0) param <- "condes"
    if(input$catdesmethod!=0) param <- "catdes"
    if(input$PCAmethod!=0) param <- "PCA"
    if(input$CAmethod!=0) param <- "CA"
    if(input$MCAmethod!=0) param <- "MCA"
    if(input$MFAmethod!=0) param <- "MFA"
    if(input$FAMDmethod!=0) param <- "FAMD"
    if(input$HCPCmethod!=0) param <- "HCPC"
    return(param)
  }  
  
  observe({
    if((input$catdesmethod!=0) |(input$condesmethod!=0) |(input$PCAmethod!=0) |(input$CAmethod!=0) |(input$MCAmethod!=0) |(input$MFAmethod!=0) | (input$FAMDmethod!=0) |(input$HCPCmethod!=0) ){
      isolate({
    ...
    output$def_pca <- renderUI({
      if (input$action_pca %% 2){
        ...
        HTML("L'analyse en composantes principales (ACP) permet de décrire et visualiser des tableaux de données
           ...
           <li><a href='https://www.youtube.com/watch?v=IQJr3hbYGn8' target='_blank'>Etude des individus et des variables</a></li>
           <li><a href='https://www.youtube.com/watch?v=Oi4aQcUb1Yc' target='_blank'>Aides à l'interprétation</a></li>
