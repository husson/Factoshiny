...
#      Call1=as.name(paste("res.HCPC<-HCPC(",nomDataHCPCshiny,",nb.clust=",input$clust,",consol=",input$consoli,",graph=FALSE,metric='",input$metric,"')",sep="")) 
  ...
  # )
    ...
    # content = function(file) {
        # path.aux <- getwd()
        # setwd(pathsaveHCPCshiny)
        # if (gettext(input$choixLANG)==gettext("English")) FactoInvestigate::Investigate(values()$res.HCPC, openFile=FALSE, file = "Investigate", language="en")
        # if (gettext(input$choixLANG)==gettext("French")) FactoInvestigate::Investigate(values()$res.HCPC, openFile=FALSE, file = "Investigate", language="fr")
